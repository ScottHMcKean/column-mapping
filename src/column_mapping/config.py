...
def compute_effective_config(
    *,
    ...
    cfg_catalog = (catalog or "").strip() or _deep_get(
        config, ["databricks", "catalog"], "main"
    )
    cfg_schema = (schema or "").strip() or _deep_get(
        config, ["databricks", "schema"], "column_mapping_demo"
    )
    cfg_prefix = (table_prefix or "").strip() or _deep_get(
        config, ["databricks", "table_prefix"], "silver"
    )

    default_rules_name = _deep_get(
        config, ["tables", "rules_table_name"], "governance_standardization_rules"
    )
    ...
    if not cfg_vs_endpoint_name:
        raise ValueError(
            "Vector Search endpoint name must be set. "
            "Configure 'vector_search.endpoint_name' in config.yaml or pass 'vs_endpoint_name'."
        )

    index_name_default = _deep_get(
        config,
        ["vector_search", "index_name"],
        "governance_standardization_mappings_index",
    )
    idx = (vs_index_name_or_full or "").strip() or index_name_default
    cfg_vs_index_full_name = (
        idx if idx.count(".") == 2 else f"{cfg_catalog}.{cfg_schema}.{idx}"
    )

    cfg_embedding_model_endpoint = (
        embedding_model_endpoint or ""
    ).strip() or _deep_get(
        config, ["vector_search", "embedding_model_endpoint"], "databricks-bge-large-en"
    )

    cfg_pk = _deep_get(
        config, ["vector_search", "delta_sync", "primary_key"], "mapping_id"
    )
    cfg_embed_col = _deep_get(
        config,
        ["vector_search", "delta_sync", "embedding_source_column"],
        "platform_header",
    )
    cfg_pipeline_type = _deep_get(
        config, ["vector_search", "delta_sync", "pipeline_type"], "TRIGGERED"
    )

    cfg_llm_endpoint = (llm_endpoint or "").strip() or _deep_get(
        config, ["llm", "endpoint"], "databricks-claude-haiku-4-5"
    )

    cfg_top_k = (
        top_k if top_k is not None else int(_deep_get(config, ["mapping", "top_k"], 5))
    )
